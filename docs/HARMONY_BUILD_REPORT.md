...
### 5.1 可复现性（2026-09-15 接力轮实测）—— HAP 哈希**不可复现**

本轮真实重建了一次（`hvigor BUILD SUCCESSFUL in 59 s 267 ms`），结论必须如实记录：

- **字节数完全相同**（18,986 B）；
- **zip 内每个条目的字节数逐项相同**（下表所有数值与首次构建一致）；
- **但整体 SHA-256 不同** —— 重建件为
  `ceeb5c5611aef313a243d12a0e428fce8af3817bb3226983a42d1dec3be1b3af`。

根因：HAP 是 zip，打包时会写入**构建时间戳**等元数据（实测条目时间为构建时刻），
因此同一份内容两次打包的字节流不相等。这与 Android 的 AAR 形成对照 ——
AAR 由 Gradle 以确定性方式打包，`--rerun-tasks` 后可**逐字节复现**。

**结论**：`platforms/harmonyos/artifacts/` 下的 HAP 现在是本轮重建件（`ceeb5c56…`）；
上表的 `4f10d059…` 是**那一件交付产物**的哈希。判据应改为
「构建成功 + 条目内容一致」，**不应**把 HAP 的 SHA-256 当作构建不变量。
