package com.depmap.core.schema

/**
 * DepMap Schema V1 — 与 Core (core/src/schema/migrations.ts) 的 DDL 逐语句一致。
 * 三端逻辑 Schema 必须一致（CANONICAL_DESIGN §8.2）。
 * 变更只允许 correctness/security/interoperability 修订，且三端同步。
 *
 * 状态：IMPLEMENTED（与 Core DDL 同步维护）。COMPILED = NO（见 BLOCKERS.md）。
 */
object DepmapSchemaV1 {
    const val VERSION = 1

    fun migrations(): List<Pair<Int, List<String>>> = listOf(
        VERSION to STATEMENTS
    )
