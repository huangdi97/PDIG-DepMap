...
describe('MVP03 Living Graph invariants（INV-15..21）', () => {
  ...
  it('INV-15: 任何序列后 ChangePlan.lastAnalyzedGraphRevision ≤ 当前 graphRevision', () => {
    const plan = instantiateScenario(driver, 'replace_payment_card', {
      targetPaymentInstrumentId: card,
    })
    ...
    expect(plans.getExisting(plan.id).lastAnalyzedGraphRevision).toBeLessThanOrEqual(
  ...
  it('INV-17: completed plan 不被 rebase 改写历史', () => {
    ...
    deps.confirm({
      from: card,
      relation: 'funding_source',
      to: nodes.create({ kind: 'account', name: 'Y' }).id,
      capability: 'payment',
    })
    rebasePlan(driver, graphRepos(), plan.id)
  ...
  it('INV-20: TimelineItem 必须存在有效 source reference', () => {
    ...
    for (const item of items) {
      switch (item.sourceType) {
        ...
        case 'reality_drift':
          expect(
            new RealityDriftService(driver).listOpen().some((d) => d.id === item.sourceId),
          ).toBe(true)
          break
        case 'node_expiry':
