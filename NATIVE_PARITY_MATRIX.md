...
> Harmony 合计 27 → 29 / 73**。证据（本轮新鲜实跑）：
>
> 1. clean `assembleHap` → `BUILD SUCCESSFUL`，HAP 3,511,091 B（sha256 `1213f05a…`，见 NATIVE_MIGRATION_STATUS.md）；
> 2. `check-compiled-reachability.mjs --build` → **35/35 required 模块 PASS**（4 个新模块全可达 + 入 abc）；
> 3. `run-conformance-host.mjs` → **122/122 host checks**（87 canonical + 3 元测试 + 1 domain 自检 +
>    14 persistence + **17 repository**：logical-key 唯一 / verify 不重复建边不 bump / retire-reactivate 同 id /
>    retire 幂等 / criticality 缺省 unknown 且永不自动 required / revision 同事务 / group 回填 /
>    proposal UPSERT 累计·accepted 不重问·rejected 需新证据 / evidence·fingerprint 唯一键 /
>    payload export-import 逐字节往返 / import 失败原子回滚 / 事务注入失败整体回滚 /
>    迁移链 ×50 幂等·legacy 一次·revision 不 bump / 迁移失败停在旧版本）。
>
> **诚实边界**：ArkData 物理 DB、crypto 哈希、change_plans/reality_drifts/discovery_candidates
> 行数据属设备层 → 相关格保持 ICNR / NOT_STARTED；主机测试不冒充设备运行时；
> `RUNTIME_E2E` 仍为 `RUNTIME_NOT_RUN`（E-9 外部依赖不变）。
...
| Migration v1→v2→v3          | **CONFORMANCE_PASS**                                                                  | TESTED（host：MigrationChain 1→2→3 ×50 幂等、legacy 只建一次、revision 不 bump；DB 物理绑定需设备）                                                | NOT_STARTED |
| 迁移失败回滚（同事务）      | **RUNTIME_VERIFIED**（设备内：失败后不推进版本号、不留半迁移状态）                    | TESTED（host：failAtStep 注入 → 停在旧版本、无半迁移状态、v1→v2 与 v2→v3 一并回滚；DB 事务绑定需设备）                                             | NOT_STARTED |
...
| Harmony | **29**      | 73     | N3 推进轮（2026-09-25）：round#1 host conformance 87 canonical PASS → §1 12 + §3 2 + §4 7 = CONFORMANCE_PASS(host) + §6 1 TESTED；round#2 ArkTS 持久化逻辑层 → §2 5 TESTED + 2 ICNR；round#3 内存持久化执行层（GraphStore/GraphRepository/MigrationChain）→ §2 再 +2 TESTED（Migration v1→v3、迁移失败回滚），合计 **29/73**。**RUNTIME 类一格未动**（RUNTIME_E2E=RUNTIME_NOT_RUN，华为账号+镜像 E-9）；ArkData DB / 加密库 / UI 仍 NOT_STARTED |
| iOS     | 0           | 73     | 仅 codegen 产物；build BLOCKED_BY_MACOS（PAUSED / NOT_STARTED）                                                                                                                                                                                                                                                                                                                                                                                 |

### Harmony 29/73 的来源（2026-09-25，逐格以本轮新鲜证据归位，无证据不升级）
...
| 2. 持久化与迁移       | 10     | **7**  | round#2 ArkTS 持久化逻辑层（SchemaV3/PayloadCodec/PayloadMigration/SourceInstance + 14 测试）+ round#3 内存持久化执行层（GraphStore/GraphRepository/MigrationChain + 17 测试）→ 逻辑 Schema / payload 导出·导入 / payload 内存迁移 / 未来版本拒绝 / SourceInstance / **Migration v1→v3 链** / **迁移失败回滚** = TESTED(host)；Fingerprint 作用域隔离、Evidence 多源分流 = IMPLEMENTED_COMPILED_NOT_RUN（哈希与 repository 层需设备）。ArkData 加密库一格保持 NOT_STARTED                                                                     |
