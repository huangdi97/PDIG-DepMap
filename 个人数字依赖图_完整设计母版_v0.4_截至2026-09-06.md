...
| 层 | 方案 |
|---|---|
| UI / 业务前端 | Web 技术栈，复用到 Capacitor |
| Android / iOS | Capacitor 原生壳 |
| 本地数据库 | SQLite + SQLCipher |
| 原生密钥安全 | iOS Keychain / Android Keystore（优先使用 `@capacitor-community/sqlite` 已验证 secure-store 能力） |
| Web 加密 | bundled Argon2id WASM + WebCrypto AES-GCM |
| Web 持久化 | **不持久化解密后的业务数据** |
| 解析器 | 本地纯解析，账单在内存处理 |

### 4.2 端优先级

1. **Android APK**：开发和迭代最快，先做真机安全 Spike。
2. **iOS**：复用主要业务代码，验证 Keychain / biometric / local notification。
3. **Web 工作台**：无状态、适合大批量录入和账单体检。
...
### 8.2 本地数据库：SQLCipher + 系统安全区

```text
随机 SQLCipher key
→ iOS Keychain / Android Keystore / 库提供的 secure store
→ 系统生物识别 / 设备凭证 gating
```

不使用 App 自己的 6 位 PIN 作为数据库密钥根；不做字段级选择性加密。

优先采用 `@capacitor-community/sqlite` 已验证的 SQLCipher / secret-store 能力，不为了架构形式自行重造密码学。
