...
>
> **2026-09-15 接力轮再修正**：B10 上一轮写的「CLI 无 build 命令」**是错的** ——
> `cli pack` 是 DCloud 官方文档定义、**明确支持 uni-app x** 的云打包命令，
> 且 `cli user login` 提供无头登录入口。真正的闸门是 **DCloud 账号**（属账号类，
> 不是工具链缺失）。另外 UTS 层已由本轮独立验证（`npm run check:uts` → 15/15），
> B10 不再覆盖 UTS，只覆盖 `.uvue` 页面编译与产品级打包。

| #       | Blocker                  | 现状（2026-09-15 实测）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 谁来提供            | 提供后执行                                                                                                                                                                                                                 | 影响 Gate                                                                     |
| ------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| **B1**  | Android **产品级**构建   | **原生层已 PASS**：JDK 21 + Android SDK + Gradle 8.9 齐备；AAR ×2 已产出（本轮 `--rerun-tasks` **逐字节复现**），黄金向量 8/8 PASS。**残留**：无 APK/AAB（需 HBuilderX）、无 keystore、无设备                                                                                                                                                                                                                                                                                                          | 用户 / CI           | 按 `docs/ANDROID_RELEASE_RUNBOOK.md`：HBuilderX 打包出 APK → 提供 keystore → 真机 `adb install` → 冒烟 → 上传 AAB                                                                                                          | ANDROID_PRODUCT_BUILD_READY / INSTALL_READY / DEVICE_VERIFIED                 |
...
| **B10** | uni-app x App 打包       | HBuilderX **5.24.2026081301**（实际路径 `<DEPMAP_TOOLS_HOME>\HBuilderX`，旧记录路径错误）。**`cli pack` 存在且官方支持 uni-app x**（`hx.dcloud.net.cn/cli/pack`）。本机 `cli pack --help` 报「命令不存在」的原因是**打包插件未安装**（33 个插件中缺 `launcher` / `uniappx-launcher` / `uniapp-cli` / `uniapp-uts-v1`，这些插件在公共 npm 上确实存在）。**准确分类 = AUTH（账号）**：云打包需 DCloud 登录，本地打包的资源生成是 GUI 步骤且 AppKey 亦需账号。**UTS 层已不再受影响**（本轮已验证 15/15） | 用户（DCloud 账号） | `cli open` → `cli user login` → `cli project open --path "<repo>\app"` → `cli pack --project app --platform android --android.packagename <正式包名> --android.androidpacktype 1`；或 GUI「发行 → 原生App-云打包」 | UI_COMPILED / UI_RUNTIME_VERIFIED / ANDROID_BUILD_READY / HARMONY_BUILD_READY |
